...
async def create_project(
    ...
    try:
        created_project = await crud_project.create_project(db, project)
...
async def get_projects(
    ...
    projects = await crud_project.get_all_projects(db, skip=skip, limit=limit)
    return [
        ProjectResponse(
            id=str(project.id),
...
async def get_project(project_id: str, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    return ProjectResponse(
        id=str(project.id), project_name=project.project_name, description=project.description,
...
async def update_project(project_id: str, project_update: ProjectUpdate, db: AsyncIOMotorDatabase = Depends(get_db)):
    ...
    updated_project = await crud_project.update_project(db, project_id, update_data)
    if not updated_project:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found")
    return ProjectResponse(
        id=str(updated_project.id), project_name=updated_project.project_name, description=updated_project.description,
