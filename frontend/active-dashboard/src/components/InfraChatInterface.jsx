...
const markdownComponents = {
  h1: ({ children }) => (
    <h1 className="text-xl font-bold mt-5 mb-3 leading-tight">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-lg font-bold mt-4 mb-2 leading-tight">{children}</h2>
  ),
  h3: ({ children }) => (
        ...
        <div className="my-4 p-4 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200/60 rounded-xl flex items-center justify-between shadow-sm">
          <div>
            ...
            <p className="text-[11px] text-emerald-600/80 mt-0.5 font-medium">
              Ready to provision the requested resources to AWS.
            </p>
          </div>
  ...
  ul: ({ children }) => (
    <ul className="list-disc pl-6 mb-3 space-y-1.5 marker:opacity-60">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal pl-6 mb-3 space-y-1.5 marker:opacity-60">
      {children}
    </ol>
  ),
  ...
  blockquote: ({ children }) => (
    <blockquote className="border border-current/20 px-4 py-2 my-3 bg-current/5 italic rounded opacity-90">
      {children}
    </blockquote>
  ),
  a: ({ href, children }) => (
  ...
  thead: ({ children }) => (
    <thead className="text-xs uppercase bg-current/5 border-b border-current/20">
      {children}
    </thead>
  ),
  tbody: ({ children }) => <tbody>{children}</tbody>,
  tr: ({ children }) => (
    <tr className="border-b border-current/10 last:border-0 hover:bg-current/5">
      {children}
    </tr>
  ),
  ...
  code({ node, inline, className, children, ...props }) {
      ...
      return (
        <div className="my-4 border border-emerald-200/60 rounded-xl overflow-hidden bg-emerald-50/30 shadow-sm">
          <div className="bg-emerald-100/50 px-4 py-2.5 border-b border-emerald-200/60 flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold bg-white text-emerald-600 px-2 py-0.5 rounded-md border border-emerald-200 shadow-sm">
              Artifact
            </span>
          </div>
    ...
    return !inline && match ? (
      ...
      <code
        className="bg-current/10 px-1.5 py-0.5 rounded text-[13px] font-mono border border-current/20"
        {...props}
      >
        {children}
      </code>
...
const InfraChatInterface = ({
  ...
  // --- Deleted infra history state ---
  const [deletedInfraHistory, setDeletedInfraHistory] = useState(null);
  const [isDeletedHistoryOpen, setIsDeletedHistoryOpen] = useState(false);

  // --- Chat state ---
  const [messages, setMessages] = useState([
    createDefaultMessage(project.project_name),
  ]);
  ...
  const COMMANDS = [
    ...
    {
      cmd: "/cost-estimate",
      desc: "Estimate AWS costs for current architecture",
    },
    { cmd: "/clear", desc: "Clear the current conversation context" },
    { cmd: "/plan", desc: "Generate a Terraform plan" },
  ];

  const MENTIONS = [{ tag: "@project", desc: "Entire project context" }];
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      ...
      // 2. Check for deleted infrastructure history
      try {
        const historyRes = await getDeletedInfraHistory(projectId);
        if (historyRes?.has_history) {
          setDeletedInfraHistory(historyRes.history);
        }
      } catch (err) {
        console.warn("Could not check deleted infra history:", err);
      }
      ...
      try {
        ...
        console.warn(
          "Could not load conversations from API, using local cache:",
          err,
        );
      }
      ...
      if (cached.length > 0) {
        ...
        setActiveSessionId(latest.id);
        setMessages(
          latest.messages || [createDefaultMessage(project.project_name)],
        );
  ...
  useEffect(() => {
    if (activeSessionId) {
      const draft =
        localStorage.getItem(`draft_${projectId}_${activeSessionId}`) || "";
      setInputValue(draft);
      setTimeout(() => {
      ...
      setTimeout(() => {
        if (textareaRef.current) {
          ...
          textareaRef.current.selectionStart =
            newTextBefore.length + textToInsert.length + 1;
  ...
  const triggerMessage = (
    ...
    if (targetSessionId && !isHidden)
      persistSession(targetSessionId, newMessages);
    ...
    if (selectedRepo) {
      ...
      if (selectedRepo.credential_id)
        paramsObj.credential_id = selectedRepo.credential_id;
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (data.type === "plan") {
            ...
            scrollToBottom();
          } else if (data.type === "workbook_update") {
            setChatSessions((prev) =>
              prev.map((s) =>
                s.id === targetSessionId
                  ? {
            ...
            setMessages((prev) => {
              ...
              if (targetSessionId)
                persistSession(targetSessionId, updatedMessages);
              return updatedMessages;
            });

            // Update session-level plan/workbook states based on final done event
            setChatSessions((prev) => {
              return prev.map((s) => {
                if (s.id === targetSessionId) {
                  const sUpdate = { ...s, updatedAt: new Date().toISOString() };
                  if (data.plan) {
                    sUpdate.plan = data.plan;
                    sUpdate.plan_status = data.plan_status || "draft";
                  }
                  if (data.workbook) {
                    sUpdate.workbook = data.workbook;
                    if (data.plan_status)
                      sUpdate.plan_status = data.plan_status;
                  }
                  return sUpdate;
                }
                return s;
            ...
            if (
              ...
              data.response_type === "plan_completed"
            ) {
  ...
  const handleStopGenerating = () => {
    if (eventSourceRef.current) {
      ...
      setMessages((prev) => {
        const finalContent = streamingMessage
  ...
  const renderGroup = (label, sessions) => {
    ...
    return (
      <div key={label} className="mb-3">
        <p className="text-[11px] font-medium text-slate-400 px-3 mb-1">
          {label}
        </p>
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-2xl flex h-[calc(100vh-7rem)] min-h-[600px] overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-1.5 pb-4">
          ...
          {/* ── Deleted Infrastructure History ── */}
          {deletedInfraHistory && (
            <div className="mx-1.5 mt-4 border-t border-slate-200 pt-3">
              <button
                onClick={() => setIsDeletedHistoryOpen(!isDeletedHistoryOpen)}
                className="w-full flex items-center gap-2 px-2.5 py-2 text-left rounded-lg hover:bg-amber-50 transition-colors group"
              >
                ...
                <span className="text-[11px] font-semibold text-amber-700 uppercase tracking-wider flex-1">
                  Deleted Infra
                </span>
                <svg
                  className={`w-3.5 h-3.5 text-slate-400 transition-transform duration-200 ${isDeletedHistoryOpen ? "rotate-180" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {isDeletedHistoryOpen && (
                <div className="mt-1.5 mx-1 p-3 bg-amber-50/70 border border-amber-200/60 rounded-lg">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-[10px] text-amber-600 font-medium">
                      {deletedInfraHistory.deleted_at
                        ? `Archived ${new Date(deletedInfraHistory.deleted_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}`
                        : "Previously deleted"}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-600 leading-relaxed whitespace-pre-line line-clamp-6">
                    {deletedInfraHistory.content || "No details available."}
                  </p>
                  {deletedInfraHistory.content &&
                    deletedInfraHistory.content.length > 300 && (
                      <details className="mt-2">
                        <summary className="text-[10px] text-amber-600 font-medium cursor-pointer hover:text-amber-700">
                          View full archive
                        </summary>
                        <p className="mt-1.5 text-[11px] text-slate-600 leading-relaxed whitespace-pre-line">
                          {deletedInfraHistory.content}
                        </p>
                      </details>
                    )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Header */}
        <div className="px-4 sm:px-6 py-2 border-b border-slate-100 bg-white/50 flex items-center justify-between flex-shrink-0">
        ...
        >
          {messages.map((msg, index) => {
            ...
            return (
              <div
                key={msg.id}
                className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"} group/msg max-w-4xl mx-auto w-full`}
              >
                <div
                  className={
                    msg.sender === "user"
                ...
                >
                  ...
                  {/* Embedded Plan Components */}
                  {isLastAiMessage &&
                    chatSessions.find((s) => s.id === activeSessionId)
                      ?.plan && (
                      <div className="w-full mt-4 max-w-4xl animate-fade-in-up">
                        <ImplementationPlanCard
                          ...
                          onConfigChange={(order, key, value) => {
                            // Update the plan in session state locally
                            setChatSessions((prev) =>
                              prev.map((s) => {
                                if (s.id === activeSessionId && s.plan) {
                                  const updatedResources = s.plan.resources.map(
                                    (r) =>
                                      r.order === order
                                        ? {
                                          ...r,
                                  ...
                                  return {
                                    ...s,
                                    plan: { ...s.plan, resources: updatedResources },
                                  };
                                }
                                return s;
                              }),
                          ...
                          onApprove={async (approvedOrders, configEdits) => {
                            // If there are config edits, send them to the backend first
                            if (configEdits && configEdits.length > 0) {
                              try {
                                const token = getToken();
                                await fetch(
                                  `${INFRA_API_URL}/api/infra/plan/config`,
                                  {
                                    method: "PATCH",
                                    headers: {
                                    ...
                                    body: JSON.stringify({
                                      session_id:
                                        ...
                                        `infra-chat-${Date.now()}`,
                                      project_id: projectId,
                                      resource_updates: configEdits,
                                    }),
                                  },
                                );
                              } catch (err) {
                                console.warn(
                                  "Failed to sync config edits to backend:",
                                  err,
                                );
                              }
                            }
                            ...
                            setStreamingMessage("");
                            setIsUserScrolledUp(false);
                            // Pass the specific orders as a command to the backend by prefixing it
                            triggerMessage(
                              `[APPROVE_ORDERS] ${approvedOrders.join(",")}`,
                            );
                          }}
                        />
                      </div>
                  ...
                  {isLastAiMessage &&
                    chatSessions.find((s) => s.id === activeSessionId)
                      ?.workbook &&
                    chatSessions.find((s) => s.id === activeSessionId).workbook
                      .length > 0 && (
                      <div className="w-full mt-4 max-w-4xl animate-fade-in-up">
                        <ProvisioningWorkbook
                          ...
                          onRetry={(order) => {
                            setInputValue(`retry resource ${order}`);
                            setIsTyping(true);
                            setStreamingMessage("");
                            triggerMessage(`retry resource ${order}`);
                          }}
                        />
                      </div>
                    )}
          ...
          {isTyping && !streamingMessage && (
            <div className="flex justify-start max-w-3xl mx-auto w-full">
              <div className="flex items-center gap-1.5 py-3">
                ...
                <span
                  className="w-2 h-2 bg-slate-400 rounded-full animate-pulse-soft"
          ...
          <div className="pointer-events-auto relative max-w-4xl mx-auto">
            ...
            {(showCommands || showMentions) && (
              <div className="absolute bottom-full mb-2 left-6 w-64 max-h-48 overflow-y-auto bg-white border border-slate-200 shadow-xl rounded-xl p-1 z-50 animate-fade-in-up">
                {showCommands &&
                  COMMANDS.filter((c) =>
                    c.cmd.toLowerCase().includes(commandFilter),
                  ).map((c) => (
                    <button
                      key={c.cmd}
                ...
                {showMentions &&
                  MENTIONS.filter((m) =>
                    m.tag.toLowerCase().includes(mentionFilter),
                  ).map((m) => (
                    <button
                      key={m.tag}
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.src =
                        "https://img.icons8.com/puffy-filled/32/ffffff/sent.png";
                    }}
                  />
                </button>
              </div>
            ...
            <p className="text-center text-[10px] text-slate-400 font-medium uppercase tracking-wider mt-3 drop-shadow-sm">
              AI may hallucinate infrastructure endpoints. Verify before
              production.
            </p>
          </div>
        </div>
